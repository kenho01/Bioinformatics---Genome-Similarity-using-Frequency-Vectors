...
void CompareBacteriaParallel(int i, int j)
{
...
#pragma omp parallel
	{
		...
		int chunk_size = number_bacteria / num_threads;
		int start = thread_id * chunk_size;
		int end = (thread_id == num_threads - 1) ? number_bacteria : start + chunk_size;

		for (int i = start; i < end; i++)
		{
			for (int j = i + 1; j < number_bacteria; j++)
			{
				CompareBacteriaParallel(i, j);
			}
		}
